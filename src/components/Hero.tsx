...
export default function Hero() {
  ...
  // Si la persona prefiere movimiento reducido, cada rango de salida colapsa
  // a su primer valor (el estado de reposo en scrollY = 0): la ilustración
  // deja de reaccionar al scroll pero se sigue viendo igual que siempre.
  const range = (from: number, to: number): [number, number] =>
    prefersReducedMotion ? [from, from] : [from, to];

  // Apple-like scroll-driven animations
  const yPhone = useTransform(scrollY, [0, 600], range(0, 150));
  const scalePhone = useTransform(scrollY, [0, 600], range(1, 1.35));
  const rotateXPhone = useTransform(scrollY, [0, 600], range(20, 0));
  const opacityPhone = useTransform(scrollY, [800, 1000], range(1, 0));

  // iPhone disassembly animations
  const screenY = useTransform(scrollY, [100 + offset, 200 + offset], range(0, -50));
  const screenOpacity = useTransform(scrollY, [120 + offset, 200 + offset], range(1, 0));
  const screenScale = useTransform(scrollY, [120 + offset, 200 + offset], range(1, 1.05));

  // Old battery moves out to left
  const oldBatteryX = useTransform(scrollY, [200 + offset, 300 + offset], range(0, -150));
  const oldBatteryY = useTransform(scrollY, [200 + offset, 300 + offset], range(0, -50));
  const oldBatteryOpacity = useTransform(scrollY, [250 + offset, 300 + offset], range(1, 0));
  const oldBatteryRotate = useTransform(scrollY, [200 + offset, 300 + offset], range(0, -15));

  // New battery moves in from right
  const newBatteryX = useTransform(scrollY, [300 + offset, 400 + offset], range(150, 0));
  const newBatteryY = useTransform(scrollY, [300 + offset, 400 + offset], range(-50, 0));
  const newBatteryOpacity = useTransform(scrollY, [300 + offset, 350 + offset], range(0, 1));
  const newBatteryRotate = useTransform(scrollY, [300 + offset, 400 + offset], range(15, 0));
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-4">
          <Link
            to="/pantallas"
            className="bg-apple-dark dark:bg-white text-white dark:text-apple-dark px-8 py-4 rounded-full text-lg font-semibold shadow-xl hover:bg-black dark:hover:bg-gray-200 transition-colors text-center w-full sm:w-auto"
          >
            Cambio de Pantalla
          </Link>
          <div className="relative w-full sm:w-auto">
            <select
              onChange={(e) => {
                if (e.target.value) {
                  navigate(`/?highlight=${e.target.value}#modelos`);
                  setTimeout(() => {
                    window.dispatchEvent(new CustomEvent('highlight-model', { detail: e.target.value }));
                  }, 100);
                }
              }}
              defaultValue=""
              className="appearance-none bg-gradient-to-br from-white to-gray-50 dark:from-[#111] dark:to-[#0a0a0a] border border-gray-200/60 dark:border-gray-800/80 shadow-[0_2px_8px_rgba(0,0,0,0.02)] dark:shadow-[0_4px_12px_rgba(0,0,0,0.2)] text-apple-dark dark:text-white px-6 py-4 pr-12 rounded-full text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-apple-blue hover:bg-gray-50 dark:hover:bg-[#141414] transition-colors cursor-pointer w-full sm:w-auto h-full"
            >
              <option value="" disabled>¿Qué iPhone tienes?</option>
              {models.map(m => (
      ...
      <div className="mt-16 md:mt-24 relative w-full flex justify-center z-10 h-[380px] pointer-events-none">
        ...
        >
          {/* Ultra-subtle background depth element */}
          <div className="absolute top-0 w-[240px] h-[400px] pointer-events-none z-0 flex items-center justify-center">
            <div className="absolute w-[200px] h-[200px] rounded-full bg-[radial-gradient(circle,rgba(255,255,255,0.02)_0%,rgba(0,0,0,0)_70%)] dark:bg-[radial-gradient(circle,rgba(255,255,255,0.01)_0%,rgba(0,0,0,0)_70%)]" />
          </div>

          <div className="w-full max-w-[200px] h-[400px] bg-black rounded-[44px] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.3)] dark:shadow-[0_20px_50px_-12px_rgba(255,255,255,0.05)] border-[6px] border-[#3a3a3c] dark:border-[#2c2c2e] p-1 flex flex-col items-center relative z-10 transition-colors duration-300">
            {/* Side Buttons */}
            {/* Action Button */}
