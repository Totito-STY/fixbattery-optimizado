...
export default function ScreenDetail() {
  ...
  // Igual que en nea: el drag solo se usa como gesto para leer offset.x,
  // nunca convive con una animación de posición en % (esa mezcla era el bug).
                ...
                <div className={`relative w-full h-full overflow-hidden flex items-center justify-center flex-grow group ${images.length > 1 ? 'mb-8 sm:mb-24' : ''}`}>
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className="absolute inset-0 flex items-center justify-center p-4 cursor-grab active:cursor-grabbing touch-pan-y"
                    >
                      <Picture
                        avif={images[currentImageIndex].avif}
                        webp={images[currentImageIndex].webp}
                        alt={`Pantalla para ${model.name} - repuesto original`}
                        draggable={false}
                        onClick={() => setIsLightboxOpen(true)}
                        className="w-full h-full object-contain cursor-zoom-in drop-shadow-lg rounded-2xl select-none"
                      />
                    </motion.div>
                  </AnimatePresence>
